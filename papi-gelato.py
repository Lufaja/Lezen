...
def bolGeld(bollen):
    bolBetaal = round((float(bollen) * prijzen["bolletjes"]),2)
    global blGeld 
    blGeld = bolBetaal
    return "€" + str(("{:.2f}".format(bolBetaal)))

def bakGeld(bakjes):
    bakBetaal = round((float(bakjes) * prijzen["bakjes"]),2)
    global bkGeld
    bkGeld = bakBetaal
    return "€" + str(("{:.2f}".format(bakBetaal)))

def hoornGeld(hoorntjeS):
    hoornBetaal = round((float(hoorntjeS) * prijzen["hoorentjes"]),2)
    global hrGeld
    hrGeld = hoornBetaal
...
def toppingGeld():
    global tpGeld
    tpGeld = round((slagroom * prijzen["toppings"]["slagroom"] + sprinkels * prijzen["toppings"]["sprinkels"] + caramelHorentje * prijzen["toppings"]["caramel"]["hoorentje"] + caramelBakje * prijzen["toppings"]["caramel"]["bakje"]), 2)
    return "€" + str(("{:.2f}".format(tpGeld)))

def bonnetje():
    print('---------["Papi Gelato"]---------')
    print("")
    print("Bolletjes     " + str(aantalBollen) + " x €"+ str(prijzen["bolletjes"]) +"   = " + bolGeld(aantalBollen))
    if aantalHoorntjes > 0 and aantalBakjes > 0:
        print("Horrentje     " + str(aantalHoorntjes) + " x €"+ str(prijzen["hoorentjes"]) +"   = " + hoornGeld(aantalHoorntjes))
        print("Bakje         " + str(aantalBakjes) + " x €"+ str(prijzen["bakjes"]) +"   = " + bakGeld(aantalBakjes))
    elif aantalBakjes > 0:
        print("Bakje         " + str(aantalBakjes) + " x €"+ str(prijzen["bakjes"]) +"   = " + bakGeld(aantalBakjes))
...
def bonZakelijk():
    prijsLiter = str("{:.2f}".format(liter * prijzen["liter"]))
    print('---------["Papi Gelato"]---------')
    print("")
    print("Liters        " + str(liter) + " x €"+ str(prijzen["liter"]) +"   = €" + str(prijsLiter))
    print('                          -------- +')
    print('Totaal                    = €' + str(prijsLiter))
    print('BTW ('+ str(prijzen["btw"]) +'%)                  = €' + str("{:.2f}".format(float(prijsLiter) * prijzen["btw"]*0.01)))
